...
func test_player_hit_emitted_once_per_invuln() -> void:
	...
	var box := _make_box()
	box.set_active(true)
	box.spawn_patterns([{"pos": box.heart_position(), "vel": Vector2.ZERO, "life": 4.0}])
	var counter := {"hits": 0}
...
func test_heart_moves_and_clamps_to_box() -> void:
	...
	box._process(10.0)
	TestHelper.eq(box.heart.position.x, 167.0, "clamps at left edge")
	Input.action_release("move_left")
	Input.action_press("move_up")
	box._process(10.0)
	TestHelper.eq(box.heart.position.y, 225.0, "clamps at top edge")
