...
const ROOM_PATH := "res://scenes/rooms/Cracks.tscn"
...
func _ready() -> void:
	...
	var room := MapBuilder.build_room(parsed["grid"], GameTiles.CRACKS_STYLE)
...
func _spawn_wisp(player: Node2D) -> void:
	...
	WispState.set_area("cracks")
	print(WispDialogue.get_line("cracks"))

func _spawn_props() -> void:
	var spots: Array[Vector2] = [Vector2(240, 240)]
	for i in spots.size():
		var p := Sprite2D.new()
		p.name = "Prop" + str(i)
